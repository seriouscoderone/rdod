...
```yaml
# domain.yaml
template_version: "2.0"    # RDOD template format version — do not change
...
Protocol (REST, gRPC, TCP, etc.) is **not** part of the port definition — it is derivable from the `tier` boundary between the consumer and provider domains. A domain-to-domain boundary within the same service implies `method-call`; a service-to-service boundary implies a network protocol. The adapter layer handles protocol-specific details.
...
- `kernel://` refs without a `#fragment` are **not** required to resolve to a `domain.yaml` (kernels are external libs). `kernel://id#TypeName` refs in port contracts resolve by convention — the fragment names a type from the kernel's package
